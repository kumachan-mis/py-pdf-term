...
class PyPDFTermExtractor:
    ...
    Args:
        ...
        to split too long terms or wrongly concatenated terms. When XML Layer extracts a
        text from a table, figure or something in a PDF file, texts in them are often
        wrongly concatenated so that the text looks like one too long candidate term.
        This is used in Candidate Term Layer.
        e.g. If a PDF have a table to compare sort algorithms and the header text wanna
        mean "Selection Sort" | "Quick Sort" | "Merge Sort", the text is recognized as a
        word like "Selection Sort Quick Sort Merge Sort". This wrongly concatenated word
        must be split down into three algorithm names.

        augmenter_mapper: Mapper from an element in candidate_config.augmenters to a
        class to augment candidates. The augumentation means that if a long candidate
        term is found, sub-terms of the original term could also be candidates. This is
        used in Candidate Term Layer.
        ...
        e.g. Term-Frequency algorithm uses only frequency of occurcences of each term.
        ...
        method scores of candidate terms when method_config.method_type equals to
        "multi". A multi-domain ranking method is an ranking algorithm which calculate
        method scores with cross-domain information.
        e.g. TF-IDF algorithm uses Inverse Document Frequency, which goes to high then a
        term appears frequently in a target domain but not in the other domains. To find
        IDF, cross-domain data are required. Therefore, TF-IDF is a multi-domain ranking
        method.

        styling_score_mapper: Mapper from an element in styling_config.styling_scores to
        a class to calculate styling scores of candidate terms. Each scoring class is
        expected to focus one specific styling feature. Then different types of scores
        are combined later.

        xml_cache_mapper: Mapper from xml_config.cache to a class to provide XML Layer
        with the cache mechanism. The xml cache manages XML files converted from input
        PDF files.

        candidate_cache_mapper: Mapper from candidate_config.cache to a class to provide
        Candidate Term Layer with the cache mechanism. The candidate cache manages lists
        of candidate terms.
        ...
        StylingLayer with the cache mechanism. The styling cache manages candidate terms
        ordered by the styling scores.
    ...
    def extract(
        ...
        Args:
            ...
            classified by domain. There MUST be an element in multi_domain_pdfs whose
            domain equals to the domain. This argument is required when a multi-domain
            ranking method is to be used.
